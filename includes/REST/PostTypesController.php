...
use WpKeetech\KeetechAds\Posts\PostType;
use WP_REST_Response;
use WP_REST_Server;
use WP_Error;

/**
 * API PostTypesController class.
 *
 * @since 0.3.0
 */
class PostTypesController extends RESTController {
    ...
    public function get_items( $request ): ?WP_REST_Response {
        ...
        $post_types = $post_type->all( $args );

        foreach ( $post_types as $post_type_single ) {
            $response = $this->prepare_item_for_response( $post_type_single, $request );
            $data[]   = $this->prepare_response_for_collection( $response );
        }

        $args['count'] = 1;
        $total         = $post_type->all( $args );
