...
namespace WpKeetech\KeetechAds\Blocks;
...
class Manager {
	...
	public function register_all_blocks(): void {
        ...
        if ( ! keetech_ads()->settings->get()['enable_ai'] || empty( keetech_ads()->settings->get()['api_key'] ) ) {
            return;
        }
    ...
    protected function register_block_metas(): void {
        ...
        foreach ( $blocks as $block ) {
            $block_folder    = KEETECH_ADS_PATH . '/build/blocks/' . $block;
            $block_options   = [];
            $markup_file_path = KEETECH_ADS_TEMPLATE_PATH . '/blocks/' . $block . 'markup.php';
